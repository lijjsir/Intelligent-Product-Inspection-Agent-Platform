...
- 当前本地依赖端口统一为默认端口：MySQL `3306`、Redis `6379`、MinIO `9000/9001`、Qdrant `6333`
- Celery 当前默认 broker / result backend 都是 Redis，但默认本地栈不再启动独立 `celery-worker`
- RabbitMQ 不再作为默认依赖编排的一部分
- RAG 当前主实现使用 Qdrant HTTP API
- Langfuse 默认通过本地自托管服务接入，访问地址为 `http://127.0.0.1:3000`
...
- `PIAP_REDIS_URL=redis://127.0.0.1:6379/0`
- `PIAP_CELERY_BROKER_URL=redis://127.0.0.1:6379/0`
- `PIAP_CELERY_RESULT_BACKEND=redis://127.0.0.1:6379/0`
- `PIAP_QDRANT_URL=http://127.0.0.1:6333`
- `PIAP_S3_ENDPOINT=http://127.0.0.1:9000`
- `PIAP_LANGFUSE_ENABLED=true`
- `PIAP_LANGFUSE_HOST=http://127.0.0.1:3000`
- `PIAP_LANGFUSE_PUBLIC_KEY=pk-lf-piap-local`
- `PIAP_LANGFUSE_SECRET_KEY=sk-lf-piap-local-secret`
...
- 管理员账号：`admin@piap.local`
- 管理员密码：`piap-admin-123`
- Project Public Key：`pk-lf-piap-local`
- Project Secret Key：`sk-lf-piap-local-secret`
...
当前默认本地开发栈不启动独立 worker；如果你需要完整验证异步链路，再额外手动启动：
...
- 安装 MinIO
- 安装或接入 Langfuse
- 如需 Celery 异步执行，保留 Redis，并额外部署独立 worker 进程
