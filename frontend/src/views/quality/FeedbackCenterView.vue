...
const activeTab = ref<"all" | "pending" | "high_risk">("all");
...
async function handleDelete(id: string) {
  try {
    await ElMessageBox.confirm("删除后无法恢复，确认删除此反馈？", "删除反馈", {
      type: "warning",
      ...
      <el-table :data="store.items" v-loading="store.loading" @row-click="openDetail" class="cursor-pointer" stripe>
        ...
        <el-table-column prop="source_type" label="来源" width="100">
          <template #default="{ row }">
            <el-tag size="small" :type="row.source_type === 'result' ? 'primary' : 'info'">
        ...
        <el-table-column label="操作" width="210" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'pending' || row.status === 'processing'" link type="success" size="small" @click.stop="openResolveDialog(row.id)">解决</el-button>
      ...
      <div v-loading="detailLoading">
        <template v-if="store.currentDetail">
          ...
          <div class="flex gap-3 mt-6 pt-4 border-t border-zinc-100">
            <el-button v-if="store.currentDetail.status === 'pending' || store.currentDetail.status === 'processing'" type="success" size="small" @click="openResolveDialog(store.currentDetail.id)">标记解决</el-button>
            ...
            <el-button type="danger" size="small" plain @click="handleDelete(store.currentDetail.id)">删除</el-button>
          </div>
        </template>
      </div>
    </el-drawer>
