...
const selectedWorkItem = computed(
  () =>
    store.workItems.find((item) => item.id === selectedWorkItemId.value) ||
    store.workItems[0] ||
    null,
);
const selectedHistoryThread = computed(
  () =>
    store.threads.find((thread) => thread.id === selectedHistoryThreadId.value) ||
    store.threads[0] ||
    null,
);
...
const meetingTargets = computed(() =>
  store.targets.filter((target) => target.target_type === "meeting_room"),
);
const createTargets = computed(() =>
  store.targets.filter((target) => target.target_type === createForm.target_type),
);
...
<template>
  <section class="collab-center" aria-labelledby="collab-title">
    <header class="center-header">
      ...
      <el-button class="touch-button" type="primary" :icon="Plus" @click="openCreate"
        >发起处理请求</el-button
      >
    </header>
    ...
    <nav class="center-tabs" aria-label="协作中心视图">
      ...
      >
        {{ tab.label }}
        <span
          v-if="tab.value === 'pending' && store.workItemSummary.pending_count"
          class="tab-count"
          >{{ store.workItemSummary.pending_count }}</span
        >
      </button>
    </nav>
    ...
    <template v-if="activeTab !== 'history'">
      <div class="filter-bar">
        <el-select
          ...
          clearable
          placeholder="全部类型"
          @change="loadActiveWorkItems"
        >
          <el-option label="记忆共享" value="memory_share" /><el-option
            label="处理请求"
        ...
        <el-select
          ...
          @change="loadActiveWorkItems"
        >
          <el-option label="会议室" value="meeting_room" /><el-option
            label="成员个人"
            value="user"
          /><el-option label="组织空间" value="org_space" />
        </el-select>
        <el-select
          v-model="roomFilter"
          clearable
        ...
        >
          <el-option
            v-for="target in meetingTargets"
        ...
        <el-button
          class="touch-button"
          :icon="Refresh"
          :loading="store.loadingWorkItems"
          @click="loadActiveWorkItems"
          >刷新</el-button
        >
      </div>

      <div v-if="store.workItemError" class="error-state" role="alert">
        <span>{{ store.workItemError }}</span
        ><button type="button" @click="loadActiveWorkItems">重新加载</button>
      </div>
        ...
        <aside class="work-list" aria-label="协作工作项列表">
          ...
          >
            <span class="card-topline"
              ...
              ><el-tag size="small" :type="statusTone(item.status)" effect="plain">{{
                statusLabel(item.status)
              }}</el-tag></span
            >
            <strong>{{ item.title }}</strong>
            <span class="scope-line"
              >{{ item.source.label || "未知来源" }} → {{ item.target.label || "未知目标" }}</span
            >
            <span class="card-meta"
              >{{ item.requester_label || "未知发起人" }} ·
              {{ formatTime(item.updated_at || item.created_at) }}</span
            >
          </button>
        </aside>
        ...
        <main class="work-detail" aria-live="polite">
          ...
          <template v-else>
            <div class="detail-heading">
              <div>
                <span class="type-label">{{ itemTypeLabel(selectedWorkItem.item_type) }}</span>
                <h2>{{ selectedWorkItem.title }}</h2>
              </div>
              <el-tag :type="statusTone(selectedWorkItem.status)" effect="plain">{{
            ...
            <dl class="metadata-grid">
              <div>
                <dt>来源</dt>
                <dd>{{ selectedWorkItem.source.label || "—" }}</dd>
              </div>
              <div>
                <dt>目标</dt>
                <dd>{{ selectedWorkItem.target.label || "—" }}</dd>
              </div>
              <div>
                <dt>发起人</dt>
                <dd>{{ selectedWorkItem.requester_label || "—" }}</dd>
              </div>
              <div>
                <dt>发起时间</dt>
                <dd>{{ formatTime(selectedWorkItem.created_at) }}</dd>
              </div>
            </dl>
            ...
            <section class="detail-section">
              <div class="section-title">
                <h3>来源证据</h3>
                <RouterLink
                  v-if="selectedWorkItem.source_link?.startsWith('/')"
                  :to="selectedWorkItem.source_link"
                  >查看来源</RouterLink
                >
                <a
                  v-else-if="selectedWorkItem.source_link"
                  :href="selectedWorkItem.source_link"
                  target="_blank"
                  rel="noreferrer"
                  >查看来源</a
                >
              ...
              <div v-if="selectedWorkItem.evidence.length" class="evidence-list">
                <div
                  v-for="(evidence, index) in selectedWorkItem.evidence"
                  :key="index"
                  class="evidence-row"
                >
                  <Document /><a
                    v-if="typeof evidence.url === 'string'"
                    :href="evidence.url"
                    target="_blank"
                    rel="noreferrer"
                    >{{ compactEvidence(evidence) }}</a
                  ><span v-else>{{ compactEvidence(evidence) }}</span>
                </div>
              </div>
            ...
            <section class="detail-section">
              ...
              <ol class="history-list">
                <li v-for="(entry, index) in selectedWorkItem.history" :key="index">
                  ...
                  <div>
                    <strong>{{ statusLabel(String(entry.action || "")) }}</strong>
                    <p>
                      {{ entry.actor_label || entry.actor_id || "系统" }} ·
                      {{ formatTime(entry.at as string) }}
                    </p>
              ...
              <el-button
                ...
                :icon="
                  ['reject', 'rejected'].includes(action)
                    ? Close
                    : ['approve', 'accepted', 'done'].includes(action)
                      ? Check
                      : Select
                "
    ...
    <div v-else class="history-workbench">
      <aside class="history-list-panel" aria-label="历史会话列表">
        ...
        <button
          ...
          :class="{ selected: selectedHistoryThread?.id === thread.id }"
          @click="selectHistoryThread(thread.id)"
        >
          <strong>{{ thread.title || "历史协作会话" }}</strong
          ><span>{{ formatTime(thread.last_message_at || thread.updated_at) }}</span>
        </button>
      </aside>
      <main class="history-detail">
        ...
        <template v-else>
          <header>
            <div>
              <h2>{{ selectedHistoryThread.title }}</h2>
              <p>只读历史 · {{ selectedHistoryMessages.length }} 条记录</p>
            </div>
          ...
          <div v-else class="message-history">
            <article
              v-for="message in selectedHistoryMessages"
              :key="message.id"
              class="history-message"
            >
              <div>
                <strong>{{ message.sender_type === "system" ? "系统" : message.sender_id }}</strong
                ><time>{{ formatTime(message.created_at) }}</time>
              </div>
              <p>{{ message.content || "（无文字内容）" }}</p>
              <div v-if="message.attachments.length" class="attachment-links">
                <a
    ...
    <el-dialog
      ...
      :title="`${actionLabel(decisionAction)}工作项`"
      width="min(520px, 92vw)"
      destroy-on-close
    >
      <label class="dialog-label" for="decision-note">{{
        ["reject", "rejected"].includes(decisionAction) ? "拒绝理由（必填）" : "处理说明（选填）"
      }}</label>
      <el-input
        id="decision-note"
      ...
      />
      <template #footer
        ><el-button class="touch-button" @click="decisionVisible = false">取消</el-button
        ><el-button
          class="touch-button"
          :type="actionType(decisionAction)"
          :loading="store.handlingWorkItem"
          @click="submitDecision"
          >确认{{ actionLabel(decisionAction) }}</el-button
        ></template
      >
    </el-dialog>
    ...
    >
      <el-form label-position="top" class="request-form" @submit.prevent="submitCreate">
        <div class="form-grid">
          ...
          /></el-form-item>
          <el-form-item label="协作目标" required
            ><el-select v-model="createForm.target_id" filterable placeholder="选择目标"
              ><el-option
                v-for="target in createTargets"
                :key="target.target_id"
                :label="targetLabel(target)"
                :value="target.target_id" /></el-select
          ></el-form-item>
        </div>
        ...
        <el-form-item label="说明" required
          ><el-input
            ...
            maxlength="4000"
            show-word-limit
            placeholder="补充背景、期望结果和必要证据"
        /></el-form-item>
        <el-form-item label="来源链接"
          ><el-input
            v-model="createForm.source_link"
            placeholder="可填写会议室、检测结果或其他内部页面链接"
        /></el-form-item>
        <el-form-item label="附件">
          ...
          <div v-if="store.pendingAttachments.length" class="pending-files">
            <span v-for="file in store.pendingAttachments" :key="file.id"
              >{{ file.name
              }}<button
                type="button"
      ...
      <template #footer
        ><el-button class="touch-button" @click="createVisible = false">取消</el-button
        ><el-button
          class="touch-button"
          type="primary"
...
.filter-bar {
  display: grid;
  grid-template-columns: minmax(140px, 190px) minmax(140px, 190px) minmax(160px, 240px) 1fr auto;
  align-items: center;
  gap: 10px;
  padding: 12px 16px;
  border: 1px solid #e4e4e7;
...
.history-thread:focus-visible {
  background: #f4f4f5;
  outline: 2px solid transparent;
}
.work-card.selected,
.history-thread.selected {
  box-shadow: inset 4px 0 #18181b;
  background: #fff;
}
.card-topline {
  display: flex;
