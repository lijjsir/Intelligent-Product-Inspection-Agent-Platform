...
const props = withDefaults(defineProps<{
  ...
  embedded: false,
  title: "告警规则配置",
  subtitle: "定义告警触发条件、严重度、通知渠道和收敛策略。",
});
...
const notificationChannelsText = ref("{}");
...
function resetForm() {
  ...
  notificationChannelsText.value = "{}";
...
function openEdit(rule: AlertRule) {
  ...
  conditionConfigText.value = JSON.stringify(form.value.condition_config || {}, null, 2);
  notificationChannelsText.value = JSON.stringify(form.value.notification_channels || {}, null, 2);
  dialogVisible.value = true;
}
...
async function handleSubmit() {
  ...
  let payload: AlertRuleCreate | AlertRuleUpdate;
  try {
    payload = {
      ...form.value,
      condition_config: parseJsonField(conditionConfigText.value, "触发条件"),
      notification_channels: parseJsonField(notificationChannelsText.value, "通知渠道"),
    };
  } catch (error: any) {
    ElMessage.warning(error.message || "JSON 配置无效");
    return;
  }
  ...
  try {
    if (formMode.value === "edit" && editingRule.value) {
      await alertRuleStore.updateRule(editingRule.value.id, payload as AlertRuleUpdate);
      ElMessage.success("规则已更新");
    } else {
      await alertRuleStore.createRule(payload as AlertRuleCreate);
...
<template>
  <div :class="['rule-shell', { embedded: props.embedded }]">
    <section v-if="!props.embedded" class="hero">
    ...
    >
      <el-form :model="form" label-position="top" class="rule-form">
        ...
        <el-form-item label="触发条件 (JSON)">
          <el-input
            v-model="conditionConfigText"
            type="textarea"
            :rows="5"
            placeholder='{"metric": "latency_ms", "operator": "gt", "threshold": 3000}'
          />
          <span class="form-hint">JSON 对象格式的触发条件，如阈值、比较运算符等</span>
        </el-form-item>
        <el-form-item label="通知渠道 (JSON)">
          <el-input
            v-model="notificationChannelsText"
            type="textarea"
            :rows="5"
            placeholder='{"email": ["ops@example.com"], "webhook": "https://..."}'
          />
          <span class="form-hint">JSON 对象格式的通知渠道配置</span>
        </el-form-item>
      </el-form>
      <template #footer>
...
.rule-form .el-select,
.rule-form .el-input-number { width: 100%; }
