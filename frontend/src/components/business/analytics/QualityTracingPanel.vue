...
function sourceLabel(row: QualityTraceItem) {
  if (row.source_type === "chat") return "AI 对话回复"
  if (row.source_type === "inspection") return "质检结果"
  return row.source_type || "未知"
}

function sourceTagType(row: QualityTraceItem) {
  return row.source_type === "chat" ? "success" : "info"
}

function verdictLabel(value: string | null) {
  if (value === "pass") return "通过"
  if (value === "fail") return "不通过"
  if (value === "manual_required") return "人工复核"
  if (value === "uncertain") return "待定"
  return value || "-"
}

function verdictTagType(value: string | null) {
...
async function handleDeleteTrace(row: QualityTraceItem) {
  if (!canDeleteTrace.value) {
    ElMessage.warning("Trace 删除属于管理员治理操作，平台运营当前仅可查看和跳转")
    return
  }
  try {
    await ElMessageBox.confirm(
      "确定要删除此 Trace 吗？系统会先删除 Langfuse 远端记录，成功后再清理本地关联数据。",
      "确认删除",
...
function metaAlertTitle() {
  ...
  if (meta.langfuse_status === "ok") {
    if (meta.canonical_source === "local_fast") {
      return "Langfuse 已连接，当前优先展示本地快速 Trace；需要时可手动执行远端校验"
    }
    return meta.canonical_source === "langfuse"
      ? "Langfuse 已连接，当前列表以远端 Trace 为准"
      : "Langfuse 已连接，当前列表为本地与远端合并结果"
  }
  if (meta.langfuse_status === "error") {
    return `Langfuse 连接异常：${meta.langfuse_error || "无法读取远端 Trace"}`
  }
  if (meta.langfuse_status === "disabled") return "Langfuse 未启用，当前仅显示本地 Trace"
...
<template>
  <div class="qt-panel">
    ...
    <el-alert
      title="统计对象仅包含质检任务结果和 AI 对话 assistant 回复；meeting 暂不纳入，人类输入消息也不会出现在这里。"
      type="info"
      :closable="false"
      show-icon
    />
    ...
    <div class="qt-summary">
      <div class="qt-summary-item">
        ...
        <span class="qt-summary-label">当前 Trace</span>
      </div>
      <div class="qt-summary-item">
        <span class="qt-summary-num">{{ syncedCount }}</span>
        <span class="qt-summary-label">远端已同步</span>
      </div>
      <div class="qt-summary-item">
        <span class="qt-summary-num">{{ riskyCount }}</span>
        <span class="qt-summary-label">高风险 / 差评记录</span>
      </div>
      <div class="qt-spacer" />
    ...
    <el-table
      ...
      empty-text="暂无质量 Trace。执行质检任务或产生 AI 对话回复后会出现在这里。"
      size="small"
      class="qt-table"
      row-key="trace_id"
    >
      ...
      <el-table-column label="对象" width="120">
        <template #default="{ row }">
          <el-tag size="small" :type="sourceTagType(row)">
            {{ sourceLabel(row) }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="verdict" label="业务结论" width="110">
        <template #default="{ row }">
          <el-tag
            v-if="row.verdict"
            size="small"
            :type="verdictTagType(row.verdict)"
          >
      ...
      <el-table-column label="文本可信度" width="110" sortable prop="trust_score">
        <template #default="{ row }">
      ...
      <el-table-column label="过度肯定" width="100" prop="overconfidence">
      ...
      <el-table-column label="Langfuse" width="100">
        <template #default="{ row }">
      ...
      <el-table-column type="expand" width="40">
        <template #default="{ row }">
          <div class="qt-expand">
            ...
            <div class="qt-expand-item"><label>Session ID</label><code>{{ row.session_id || "-" }}</code></div>
            <div class="qt-expand-item"><label>Assistant Message ID</label><code>{{ row.assistant_message_id || "-" }}</code></div>
            <div class="qt-expand-item"><label>Observation ID</label><code>{{ row.observation_id || "-" }}</code></div>
...
.qt-table :deep(.el-table__body-wrapper) {
