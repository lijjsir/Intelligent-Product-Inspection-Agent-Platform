...
PAPER_FORMAT_TEMPLATES: dict[str, dict[str, Any]] = {
    "generic_cn_thesis": {
        ...
        "rule_basis": {
            ...
            "figure.*": "模板规则数据：图表应紧接首次引用的段落之后，不应出现在引用之前，不应跨多页远离引用。",
            "table.*": "模板规则数据：表格应紧接首次引用的段落之后，不应出现在引用之前，不应跨多页远离引用。",
    ...
    "cqupt_graduate_thesis_2022": {
        ...
        "rule_basis": {
            ...
            "figure.*": "模板规则数据：图题/图号与正文引用一致性检查；图表应紧接首次引用的段落之后，不应前置。",
            "table.*": "模板规则数据：表题/表号与正文引用一致性检查；表格应紧接首次引用的段落之后，不应前置。",
            "figure.placed_before_mention": "写作指南要求图表紧接首次引用的正文段落之后，不应出现在首次引用之前。",
            "table.placed_before_mention": "写作指南要求表格紧接首次引用的正文段落之后，不应出现在首次引用之前。",
            "figure.far_from_mention": "写作指南要求图不宜跨多页远离首次引用的段落。",
            "table.far_from_mention": "写作指南要求表不宜跨多页远离首次引用的段落。",
            "figure.not_referenced_in_text": "写作指南要求正文中对每个图有'如图 X 所示'等明确引用。",
            "table.not_referenced_in_text": "写作指南要求正文中对每个表有'如表 X 所示'等明确引用。",
