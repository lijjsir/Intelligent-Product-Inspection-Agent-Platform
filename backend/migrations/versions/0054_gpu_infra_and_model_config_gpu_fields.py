...
def upgrade():
    ...
    for col_name in [
        "training_command_template", "fine_tune_command_template",
        "offline_eval_command_template", "runtime_env_json",
        "default_gpu_request", "default_cpu_request", "default_memory_gb",
    ]:
        if not _has_column("model_configs", col_name):
            op.add_column("model_configs", sa.Column(col_name, sa.Text() if "template" in col_name or "json" in col_name else sa.Integer(), nullable=True))
