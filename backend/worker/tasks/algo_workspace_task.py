...
@celery_app.task(name="worker.tasks.algo_workspace_task.poll_gpu_nodes")
def poll_gpu_nodes(task_payload: dict | None = None) -> dict:
    from app.services.algo_workspace_service import poll_gpu_nodes_pipeline

    payload = task_payload or {}
    return run_celery_async(
        poll_gpu_nodes_pipeline(
            org_id=str(payload.get("org_id") or ""),
            user_id=str(payload.get("user_id") or ""),
        )
    )
