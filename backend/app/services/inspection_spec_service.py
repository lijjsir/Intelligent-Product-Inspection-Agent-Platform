...
class InspectionSpecService(TenantAwareService):
    ...
    def _serialize_spec(spec, items: list[Any]) -> dict[str, Any]:
        return {
            ...
            "product_family": getattr(spec, "product_family", None),
            "applicable_skus": list(getattr(spec, "applicable_skus", None) or []),
            "required_views": list(getattr(spec, "required_views", None) or []),
            "effective_from": getattr(spec, "effective_from", None),
            "effective_to": getattr(spec, "effective_to", None),
            ...
            "aggregation_rules": dict(getattr(spec, "aggregation_rules", None) or {}),
            "ai_gate_rules": dict(getattr(spec, "ai_gate_rules", None) or {}),
            "manual_review_policies": dict(getattr(spec, "manual_review_policies", None) or {}),
