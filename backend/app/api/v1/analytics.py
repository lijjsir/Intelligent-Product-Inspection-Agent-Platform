...
from app.schemas.analytics import ModelDrilldown, OverviewStats, ProductLineDrilldown
...
async def product_line_drilldown(
    ...
    require_role("analytics", current.role)
    service = AnalyticsService(db, current.org_id)
    stats = await service.product_line_drilldown(product_line, start_date=start_date, end_date=end_date)
...
async def model_drilldown(
    ...
    require_role("analytics", current.role)
    service = AnalyticsService(db, current.org_id)
    stats = await service.model_drilldown(model_key, start_date=start_date, end_date=end_date)
