...
@router.get("/{spec_id}", response_model=ResponseEnvelope[InspectionSpecResponse])
async def get_inspection_spec(
    ...
    require_role("inspection_spec", current.role)
    service = InspectionSpecService(db, current.org_id)
    return ResponseEnvelope(data=await service.get_spec(spec_id))
...
@router.patch("/{spec_id}", response_model=ResponseEnvelope[InspectionSpecResponse])
async def update_inspection_spec(
    ...
    require_role("inspection_spec", current.role)
    service = InspectionSpecService(db, current.org_id)
    return ResponseEnvelope(data=await service.update_spec(spec_id, payload.model_dump(exclude_unset=True), current.role))


@router.delete("/{spec_id}", response_model=ResponseEnvelope[bool])
async def delete_inspection_spec(
    ...
    require_role("inspection_spec", current.role)
    service = InspectionSpecService(db, current.org_id)
    await service.delete_spec(spec_id, current.role)
